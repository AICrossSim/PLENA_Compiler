...
from ._vector_manager import VectorManager
...
class TileTensorProgram:
    ...
    def tensor(self, name: str, logical_shape: LogicalShape) -> Tensor | Vector:
        ...
        if isinstance(tensor, Vector):
            self.vector_manager.initialize_vector_backing(tensor)
        return tensor

    def vector(self, name: str, logical_shape: LogicalShape) -> Vector:
        vector = self.vector_manager.vector(name, logical_shape)
        self.vector_manager.initialize_vector_backing(vector)
        return vector
    ...
    def alloc_fragment(
        ...
        *,
        ...
        if isinstance(fragment, Vector):
            self.vector_manager.initialize_vector_backing(fragment, init_zero=init_zero)
    ...
    def alloc_shared(
        ...
        *,
        ...
        shared = self.tensor_manager.alloc_shared(
            name=name,
            logical_shape=logical_shape,
            init_zero=init_zero,
            dtype=dtype,
        ...
        if isinstance(shared, Vector):
            self.vector_manager.initialize_vector_backing(shared, init_zero=init_zero)
        if init_zero and isinstance(shared, Tensor):
            self.clear(shared)
        return shared
    ...
    def set_vram_class(self, operand: object, vram_class: str) -> None:
        resolved_class = "shared" if str(vram_class) == "shared" else "l0"
        if hasattr(operand, "metadata") and isinstance(getattr(operand, "metadata"), dict):
            operand.metadata["vram_class"] = resolved_class
        if isinstance(operand, (TensorTile, InputTile, VectorTile)):
            tiles = [operand]
        else:
            tiles = [
                tile
                for tile in self.tensor_manager._resolve_tiles_from_operand(operand)
                if isinstance(tile, (TensorTile, InputTile, VectorTile))
            ]
        for tile in tiles:
            tile.metadata["vram_class"] = resolved_class
            if isinstance(tile, VectorTile):
                continue
            value = self.value_manager.full_tile_bindings.get(tile.tile_id)
            if value is None:
                continue
            bound_value = self.value_manager.value_tiles.get(value)
            if bound_value is not None:
                bound_value.metadata["vram_class"] = resolved_class

    def set_vram_priority(self, operand: object, priority: int) -> None:
        resolved_priority = int(priority)
        self.set_vram_class(operand, "shared" if resolved_priority > 0 else "l0")

    def clear_tensor(self, operand: object, *, weak: Optional[bool] = None) -> Optional[str] | List[str]:
        ...
        for tile in tiles:
            if not isinstance(tile, TensorTile) or isinstance(tile, VectorTile):
                raise TypeError(
                    f"clear_tensor expects Tensor/TensorSlice/TensorTile operands, got {type(tile).__name__}"
                )
    ...
    def free_tensor_tile(self, operand: object, *, weak: Optional[bool] = None) -> Optional[str] | List[str]:
        return self.clear_tensor(operand, weak=weak)
    ...
    def map_tile_to_fp_fragment(
        ...
        return self.vector_manager.bind_tile_to_fp_fragment(tile, fragment)

    def _initialize_vector_backing(self, vector: Vector, *, init_zero: bool = False) -> None:
        self.vector_manager.initialize_vector_backing(vector, init_zero=init_zero)
    ...
    def _active_fp_fragments_snapshot(self) -> List[str]:
        fragment_names = {
            fragment_name
            for fragment_name in self.vector_manager.fp_fragment_bindings.values()
        ...
        for thread_index, thread in enumerate(signal_1):
            ...
            write_state = self.btmm_write(
                ...
                metadata={
                    ...
                    "vram_class": dst_tiles[0].metadata.get("vram_class", "l0"),
                },
    ...
    def _row_op_vector_tile(
        ...
        *,
        ...
        fragment = self.vector_manager.resolve_fp_fragment(src_tile)

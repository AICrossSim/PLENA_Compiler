...
class ValueManager:
    ...
    def bind_tile_to_fp_fragment(self, tile: VectorTile, fragment: FPFragment) -> FPFragment:
        return self.program.vector_manager.bind_tile_to_fp_fragment(tile, fragment)

    def resolve_fp_fragment(self, tile: VectorTile) -> FPFragment:
        return self.program.vector_manager.resolve_fp_fragment(tile)
    ...
    def _tile_vram_class(self, tile_id: str) -> str:
        tile = self.program.tensor_manager.tensor_tiles.get(tile_id)
        if tile is None:
            tile = self.program.tensor_manager.input_tiles.get(tile_id)
        if not isinstance(tile, (TensorTile, InputTile, VectorTile)):
            return "l0"
        raw_class = tile.metadata.get("vram_class", "l0")
        if raw_class == "shared":
            return "shared"
        return "l0"

    def _value_tile_ref_counts(self, value_tile_id: str) -> Dict[str, int]:
        ref_counts = {"shared": 0, "l0": 0}
        for tile_id in self.value_tile_tensor_refs.get(value_tile_id, set()):
            ref_counts[self._tile_vram_class(tile_id)] = ref_counts.get(self._tile_vram_class(tile_id), 0) + 1
        return ref_counts

    def _metadata_vram_class(self, metadata: Optional[Dict[str, object]]) -> str:
        if metadata is None:
            return "l0"
        raw_class = metadata.get("vram_class", "l0")
        if raw_class == "shared":
            return "shared"
        return "l0"

    def _value_tile_vram_protection_score(self, value: Optional[ValueTile]) -> int:
        if value is None:
            return 0
        ref_counts = self._value_tile_ref_counts(value.value_tile_id)
        shared_refs = int(ref_counts.get("shared", 0))
        total_refs = sum(int(count) for count in ref_counts.values())
        return shared_refs * 1000 + total_refs * 10

    def _request_vram_protection_score(
        self,
        *,
        value: Optional[ValueTile] = None,
        metadata: Optional[Dict[str, object]] = None,
        ...
        if value is not None:
            live_score = self._value_tile_vram_protection_score(value)
            if live_score > 0:
                return live_score
            source_tile_id = value.metadata.get("source_tile_id")
            if isinstance(source_tile_id, str) and self._tile_vram_class(source_tile_id) == "shared":
                return 1010
            if self._metadata_vram_class(value.metadata) == "shared":
                return 1010
            return 10
        if self._metadata_vram_class(metadata) == "shared":
            return 1010
        return 10

    def _vram_eviction_candidates(self, *, max_score: Optional[int] = None) -> List[str]:
        arrival_order = {value_tile_id: index for index, value_tile_id in enumerate(self._value_tiles_in_vram.keys())}
        candidates: List[str] = []
        for value_tile_id in self._value_tiles_in_vram.keys():
            if self._is_protected_value_tile(value_tile_id, "vram"):
                continue
            value = self.value_tiles.get(value_tile_id)
            if value is None:
                continue
            score = self._value_tile_vram_protection_score(value)
            if max_score is not None and score > int(max_score):
                continue
            candidates.append(value_tile_id)
        candidates.sort(
            key=lambda value_tile_id: (
                self._value_tile_vram_protection_score(self.value_tiles.get(value_tile_id)),
                arrival_order.get(value_tile_id, 0),
            )
        )
        return candidates

    def _eviction_count_needed_for_vram_request(self, tile_count: int) -> int:
        capacity = getattr(self.program, "vram_tile_capacity", 0)
        if capacity <= 0:
            return 0
        return max(0, len(self._value_tiles_in_vram) + int(tile_count) - capacity)
    ...
    def evaluate_contiguous_vram_value_tile_window(
        ...
        *,
        ...
        request_score = self._request_vram_protection_score(metadata=metadata)
        eviction_count = self._eviction_count_needed_for_vram_request(tile_count)
        eviction_candidates = self._vram_eviction_candidates(max_score=request_score)
        eviction_penalty = 0
        if eviction_count > 0:
            if len(eviction_candidates) < eviction_count:
                eviction_penalty = 10**12
            else:
                eviction_penalty = sum(
                    1 + max(0, self._value_tile_vram_protection_score(self.value_tiles.get(value_tile_id)))
                    for value_tile_id in eviction_candidates[:eviction_count]
                )
        ...
        candidates.append(
            {
                ...
                "size": window_size,
                "cost": window_size + eviction_penalty,
                "waste": 0,
                "block_name": "<bump>",
                "eviction_count": eviction_count,
                "eviction_penalty": eviction_penalty,
    ...
    def allocate_contiguous_vram_value_tiles(
        ...
        *,
        ...
        plan = self.evaluate_contiguous_vram_value_tile_window(
            tile_count=tile_count,
            metadata=metadata,
            reason=reason,
        )
        request_score = self._request_vram_protection_score(metadata=metadata)
        self._evict_fifo_if_needed("vram", required_tiles=tile_count, max_score=request_score)
    ...
    def allocate_value_tile_address(
        ...
        *,
        ...
        if place == "vram":
            request_score = self._request_vram_protection_score(value=value_tile)
            self._evict_fifo_if_needed("vram", required_tiles=1, max_score=request_score)
    ...
    def _evict_fifo_if_needed(
        self,
        place: str,
        *,
        required_tiles: int = 1,
        max_score: Optional[int] = None,
    ) -> None:
        if place == "vram":
            capacity = getattr(self.program, "vram_tile_capacity", 0)
            if capacity > 0:
                while len(self._value_tiles_in_vram) + int(required_tiles) - 1 >= capacity:
                    self._evict_one_value_tile("vram", max_score=max_score)
            return
        if place == "mram":
    ...
    def _evict_one_value_tile(self, place: str, *, max_score: Optional[int] = None) -> None:
        ...
        if place == "vram":
            resident_ids = self._vram_eviction_candidates(max_score=max_score)
            if not resident_ids:
                detail = "" if max_score is None else f" with max_score={int(max_score)}"
                raise RuntimeError(
                    f"{place.upper()} allocation requested but no resident value tile was available for FIFO eviction{detail}"
                )
            skipped_protected = 0
            while resident_ids:

...
class TensorManager:
    ...
    def alloc_fragment(
        ...
        *,
        ...
        return self._alloc_tensor_storage(
            name=name,
    ...
    def _alloc_tensor_storage(
        ...
        *,
        ...
        if len(logical_shape) == 4:
            ...
            tensor.metadata["vram_class"] = vram_class
            for tile in tensor.tiles.values():
                tile.metadata["vram_class"] = vram_class
            return tensor
        if len(logical_shape) == 3:
            ...
            vector.metadata["vram_class"] = vram_class
            for tile in vector.tiles.values():
                tile.metadata["vram_class"] = vram_class
            return vector
        raise NotImplementedError(
            f"tensor storage allocation supports 4D tensor fragments and 3D vector fragments only, got {logical_shape}"
        )
    ...
    def mapf(self, operand: object) -> List[FPVar]:
        ...
        if isinstance(operand, Vector):
            return self.program.vector_manager.resolve_vector_fp_vars(operand)
        if isinstance(operand, VectorSlice):
            return self.program.vector_manager.resolve_vector_slice_fp_vars(operand)
        if isinstance(operand, VectorTile):
            return self.program.vector_manager.resolve_vector_tile_fp_vars(operand)
    ...
    def _element_fragment_and_index(
        ...
        *,
        ...
        if isinstance(tile, VectorTile):
            fragment = self.program.vector_manager.resolve_fp_fragment(tile)
        else:
            backing_value = self.program.value_manager.resolve_value_tile(tile)
    ...
    def vector(self, name: str, logical_shape: LogicalShape) -> Vector:
        return self.program.vector_manager.vector(name, logical_shape)

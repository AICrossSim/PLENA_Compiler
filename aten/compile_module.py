...
from compiler.aten.ops.plena.norm_ops import rms_norm_plena, layer_norm_plena
from compiler.aten.ops.plena.ffn_ops import ffn_plena
from compiler.aten.ops.plena.attention_ops import flash_attention_plena
...
def _handle_add(prog, node, tensor_map):
    ...
    Computes a + b by doing b += a (in-place on b).
    Returns b_var (which now holds a + b).

    The residual-save pre-pass ensures that if either operand has downstream
    consumers, it is preserved in a fresh VRAM location before the add mutates.
    """
    a_node = node.args[0]
    b_node = node.args[1]
...
def _handle_sdpa(prog, node, tensor_map):
    ...
    """
    import math
    from compiler.aten.plena_compiler import InputVar as _InputVar
...
def compile_module(
    ...
    # In-place ops (rms_norm, layer_norm, add) destroy their input operand(s).
    # If an operand is referenced by later nodes (e.g. residual add), we must save
    # a copy before the in-place op. This pre-pass builds a set of
    # (in_place_node_name, input_arg_name) tuples that need saving.
    aten = torch.ops.aten
    _INPLACE_OPS = {aten.rms_norm.default, aten.layer_norm.default, aten.add.Tensor}
    _needs_residual_save: dict[str, list[str]] = {}  # in_place_node_name -> list of input_arg_names
    ...
    for i, node in enumerate(node_list):
        ...
        # For rms_norm/layer_norm: only args[0] is destroyed (input)
        # For add.Tensor: both args[0] (a) and args[1] (b) are potentially mutated
        #   (we do b += a, so b's location is modified)
        args_to_check = []
        if node.target in (aten.rms_norm.default, aten.layer_norm.default):
            # Only first arg (input) is mutated
            args_to_check = [node.args[0]]
        elif node.target == aten.add.Tensor:
            # Both operands are potentially used as the output; the implementation
            # does b += a, so b is the one being mutated in-place.
            # However, we should preserve both operands if they have later consumers.
            args_to_check = list(node.args[:2])

        for input_arg in args_to_check:
            if not hasattr(input_arg, "name"):
                continue
            arg_name = input_arg.name

            # Check if arg_name is used by any later node (after this in-place op)
            has_later_consumer = False
            for later_node in node_list[i + 1 :]:
                ...
                if _has_ref(all_refs, arg_name):
                    has_later_consumer = True
                    break

            if has_later_consumer:
                if node.name not in _needs_residual_save:
                    _needs_residual_save[node.name] = []
                _needs_residual_save[node.name].append(arg_name)
    ...
    for node in graph.nodes:
        if node.op == "placeholder":
            ...
            # --- Residual save: before in-place ops, preserve original variables ---
            # Strategy: for each operand that has later consumers, store the original
            # to HBM, load a fresh copy to a new VRAM location, give the fresh copy
            # to the in-place op. The original VRAM location is preserved for later
            # residual references.
            if node.name in _needs_residual_save:
                for arg_name in _needs_residual_save[node.name]:
                    if arg_name not in _residual_hbm:
                        ...
                        # We temporarily swap: tensor_map[arg_name] = fresh_copy
            ...
            if node.name in ffn_triggers:
                ...
                # --- Residual restore: after in-place op, restore original variables ---
                if node.name in _needs_residual_save:
                    for arg_name in _needs_residual_save[node.name]:

...
def test_fma_stream_lowering_removes_address_and_scalar_issue_from_loop(tmp_path):
    ...
    assert _loop_body(stream_code) == ["V_FMA_VF gp1, gp2, f1, 0"]
    assert "L_STREAM_CFG" in stream_code
...
def test_stream_lowering_covers_regular_unary_vector_and_map_loops():
    ...
    assert _loop_body(unary) == ["V_EXP_V gp1, gp1, 0"]
    assert _loop_body(binary) == ["V_ADD_VV gp1, gp1, gp2, 0"]
    assert _loop_body(mapped) == ["S_MAP_FP_V gp2, gp1, 0"]
    assert unary.count("L_STREAM_CFG") > 0
    assert binary.count("L_STREAM_CFG") > unary.count("L_STREAM_CFG")
    assert mapped.count("L_STREAM_CFG") > unary.count("L_STREAM_CFG")
    assert mapped.count("L_STREAM_CFG") < binary.count("L_STREAM_CFG")
...
def test_short_and_reverse_walks_fall_back_to_the_plain_static_path():
    ...
    assert "L_STREAM_CFG" not in short
    assert "L_STREAM_CFG" not in reverse

...
def create_mem_for_sim(data_size=256, mode="behave_sim", asm="attn", data=None, specified_data_order = None, build_path = None):
    ...
    plena_toml_path = str(_PROJECT_ROOT / "plena_settings.toml")
    ...
    if mode == "behave_sim":
        if build_path is not None:
            ...
            asm_file = Path(_PROJECT_ROOT / "behavioral_simulator" / "testbench" / "build" / "generated_asm_code.asm")
    else:
    ...
    if mode != "behave_sim":
        ...
        if build_path is not None:
            ...
            target_dir = _PROJECT_ROOT / "behavioral_simulator" / "testbench" / "build"

    ...
    if batch == 1:
        ...
        for i in range(math.ceil(hidden_size / elements_per_prefetch)):
            ...
            generated_code += _add_large_int(
                result_register, result_register, vram_step
            )
        ...
        generated_code += _add_large_int(
            result_register, result_register, vlen * preload_len * vram_stride_mult
        )
        if batch > preload_len:
            generated_code += _add_large_int(
                a_offset_register, a_offset_register, hidden_size * preload_len
            )

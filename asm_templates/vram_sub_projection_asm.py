...
def vram_sub_projection_asm_impl(
    ...
    if do_unroll:
        ...
        # A single reduction block needs no loop: the counter setup, the branch
        # and the trailing cursor advances are all dead when the body runs once.
        accumulate = (
            f"M_TMM 0, gp{gp_act}, gp{gp_mat}"
            if transposed
            else f"M_MM 0, gp{gp_mat}, gp{gp_act}"
        )
        if num_hidden_blocks == 1:

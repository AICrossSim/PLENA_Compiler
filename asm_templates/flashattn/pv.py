...
def computing_pv_code(
    ...
    v_hbm_offset: int | None = None,
    v_hbm_head_stride: int | None = None,
) -> str:
    """
    ...
    assert p_base_address + q_head_index * mlen * mlen >= 0, "p_base_address must be non-negative"
    assert v_head_index * head_dim >= 0, "v_base_address must be non-negative"
    assert output_base_address >= 0, "output_base_address must be non-negative"
    assert v_msram_base >= 0, "v_msram_base must be non-negative"
    ...
    # V HBM element offset priority:
    # 1) explicit absolute tile offset (v_hbm_offset)
    # 2) head-stride based offset (legacy helper contract)
    # 3) fallback seq-major offset
    if v_hbm_offset is not None:
        v_offset = v_hbm_offset
    elif v_hbm_head_stride is not None:
        v_offset = v_head_index * v_hbm_head_stride
    else:
        v_offset = v_head_index * head_dim
    generated_code += _load_large_int(v_base_register, v_offset)

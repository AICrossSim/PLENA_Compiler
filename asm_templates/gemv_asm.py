...
def gemv_asm(
    ...
    for weight_row in range(out_features // blen):
        ...
        for inner_loop_index in range(hidden_size // mlen):
            ...
            lines.append(f"S_ADDI_INT gp{act_reg}, gp{act_reg}, {mlen} ")
        lines.append(f"M_MV_WO gp{intermediate_register}, gp0, 0 ")
        lines.append(
        ...
        if (weight_row + 1) % (mlen // blen) == 0 and weight_row != out_features // blen - 1:
            lines.append(f"S_ADDI_INT gp{result_reg}, gp{result_reg}, {mlen} ")

...
> **Draft review only — not for merge.** This records an implemented pre-RTL
> candidate and unresolved review questions, not an architecture approval.
> Read [review scope](REVIEW_SCOPE_20260905.md) for precision and evidence limits.

## Candidate under review

The implemented candidate has one atomic Matrix-SRAM configuration form and
one deterministic execution form:
...
existing Matrix SRAM under Compiler ownership. Original PLENA already includes
Matrix accumulators. This candidate does not demonstrate that they were absent,
or that its FP32 Vector partial sums can reuse them without a new physical mapping.
...
Bit 31 marks a Matrix-view transfer and bits 30:29 name its slot. Both ordinary
and viewed transfers decode Activation=0, KV=1 and State=2. The corrected
ordinary DMA decoder no longer treats selector 2 as a KV alias; old binaries
that intended KV with selector 2 must use selector 1.
...
## Motivation and unresolved mapping
...
The optional compiler-only ordinary-ISA control in
[kda_no_extra_hardware.md](kda_no_extra_hardware.md) emits no L_TILE. It and the
FP32-dot control default off; neither has been selected as the architecture.
...
The historical guard suite covered these restrictions; for this review
checkout, see paired PR fresh validation. These checks do not imply a completed
batched full-model numerical path.
...
These are historical analytic experiment labels. Original PLENA did not publish
this Mamba/KDA implementation. The separately executable prepared A/B controls
have different instruction streams and must not inherit these baseline labels.

| Variant | Mechanism | Credit |
|---|---|---|
| A | this work's unoptimized row-by-row recurrence extension | analytic issue-proxy baseline |
...
Values below are retained from the archived campaign, without regeneration.
Use the paired Simulator PR's versioned artifacts for subsequent accounting
fixes; these historical ratios are not zero-additional-hardware measurements.
...
`A` and `B` are one-cycle-per-issued-instruction proxies for this work's unoptimized and

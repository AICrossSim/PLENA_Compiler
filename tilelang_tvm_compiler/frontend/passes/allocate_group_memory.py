...
def _analyze(func: tir.PrimFunc, lane_count: int,
    ...
    def visit(stmt, lane_var: Optional[tir.Var], gemm_kind: Optional[str]):
        ...
        if isinstance(stmt, tir.Evaluate):
            ...
            elif (op_name == _TILEOP_GEMM
                  and (gemm_kind == "overwrite" or gemm_kind is None)
                  and lane_var is not None):
                # Default (non-btmm) gemm in a lane group. We mark
                # operand lane axes only if no surrounding op (DMA /
                # btmm / extern) already did — that preserves the
                # legacy "matmul-overwrite is neutral when operands
                # are DMA-touched" contract while still expanding
                # fragment-only outputs (e.g. PV_loc in flash-attention
                # P @ V) without needing an explicit extern call.
                # Per-head layout: LHS=ROW_STACK (each lane its own
                # MLEN-wide LHS row / tile), RHS+DST=COL_PACK (each
                # lane its own hlen-wide column slice).
                lhs = _region_buffer(v.args[0])
                rhs = _region_buffer(v.args[1])
                dst = _region_buffer(v.args[2])
                for buf, mode in (
                    ...
                    if buf is not None and buf.name not in info:
                        record(buf, lane_var, lane_count, mode)

...
Valid kinds (mirrors ``frontend.gemm_macros``):
  ...
  * ``"overwrite"`` — every non-head-fused gemm. **Default when no
    annotation.** Auto-dispatches to ``plena.matmul`` or ``plena.mv``
    based on LHS rows; auto-injects per-lane offsets from buffer
    shapes; auto-marks lane axes (LHS=ROW_STACK / RHS+DST=COL_PACK)
    for operands not already marked by surrounding DMA / extern.

  * ``"btmm"`` — head-fused (Q @ K^T style). Auto-dispatches to
    ``plena.btmm`` / ``plena.btmv`` based on LHS rows.
...
# Attribute key the kernel author uses to pass a scratch buffer to a
# kind="add" gemm (since T.gemm's signature has no slot for one).
# Usage:
...
__all_extra__ = ["GEMM_SCRATCH_KEY"]

...
def lookup(name: str) -> IntrinsicSpec:
    if name not in _REGISTRY:
        raise KeyError(f"unknown PLENA intrinsic: {name!r}. Known: {sorted(_REGISTRY.keys())}")
...
register(
    IntrinsicSpec(
        ...
        operand_scopes=(
            ...
            None,
            None,
            None,  # M_tiles, K_tiles, N
            None,
            None,
            None,
            None,  # lhs_offset, rhs_offset, dst_offset, dst_row_stride
        ),
        emit=lambda a: (
...
register(
    IntrinsicSpec(
        ...
        operand_scopes=(
            ...
            None,
            None,
            None,  # lhs_offset, rhs_offset, dst_offset
        ),
        emit=lambda a: f"MV       A={a[0]}  B={a[1]}  C={a[2]}  lhs_off={a[3]}  rhs_off={a[4]}  dst_off={a[5]}",
    )
)
